...
class GeneralPaymentBackendTestCase(TestCase):
    ...
    def test_enforcing_of_name_works(self):
        ...
        with SettingsOverride(SHOP_PAYMENT_BACKENDS=MODIFIERS):
            self.assertRaises(
                NotImplementedError, backends_pool.get_payment_backends_list
            )
    ...
    def test_get_backends_from_non_path(self):
        ...
        with SettingsOverride(SHOP_PAYMENT_BACKENDS=MODIFIERS):
            self.assertRaises(
                ImproperlyConfigured, backends_pool.get_payment_backends_list
            )
...
class PayOnDeliveryTestCase(TestCase):
    ...
    def test_order_required_before_payment(self):
        ...
        self.assertEqual(
            "http://testserver/shop/cart/", response._headers["location"][1]
        )
        ...
        User.objects.create_user(
            username=username, password=pw, email="test@example.com"
        )
        ...
        response = self.client.get(reverse("pay-on-delivery"))
        self.assertTrue(
            reverse("thank_you_for_your_order") in response._headers["location"][1]
        )
